...
vim.keymap.set('n', '<leader>ff', function() builtin.find_files({ find_command = find_command }) end,
  { desc = "Search all files in directory" })
...
vim.keymap.set('n', '<leader>fe', builtin.lsp_references,
  { noremap = true, silent = true, desc = "Search symbol references" })
vim.keymap.set('n', '<leader>fdf',
  function() builtin.find_files({ find_command = find_command, cwd = '~/.dotfiles' }) end,
  { desc = "Search all files in dotfiles repo" })
...
end, { desc = "Search git commited files in repo of choice" })
...
vim.keymap.set('n', '<leader>faf', function()
  builtin.find_files({ cwd = cwdFn(), find_command = find_command });
